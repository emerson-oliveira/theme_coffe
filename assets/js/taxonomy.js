jQuery(document).ready(($) => {
  jQuery('#cmcMoreProducts').click((event) => {
    ...
    if (jQuery('#cmcSearch').val() == 1) {
      ...
      $.ajax({
        ...
        data: {
          action: 'cmc_get_next_products', postType: jQuery('#cmcPostType').val(), taxonomy: jQuery('#cmcTaxonomy').val(), term: jQuery('#cmcTerm').val(), offset: jQuery('#cmcOffset').val(),
        },
        success(result) {
          if (result != 2) {
